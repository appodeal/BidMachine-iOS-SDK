...
//__block BDMTargeting *restriction;
//
//beforeEach(^{
...
//
//    [targeting stub:@selector(restrictions) andReturn:restriction];
...
//
//    BDMBid *oneBid = BDMBid.nullMock;
//    [oneBid stub:@selector(ID) andReturn:@"id"];
//    [oneBid stub:@selector(bidfloor) andReturn:@1.1];
//    [request stub:@selector(bids) andReturn:@[oneBid]];
//
////    NSData *data = builder.message;
//
//    ORTBOpenrtb *rtb = [[ORTBOpenrtb alloc] initWithData:data error:&error];
//    ORTBRequest_Item *item = rtb.request.itemArray.firstObject;
//
//    [[error should] beNil];
//    [[item shouldNot] beNil];
//
//    [[item.id_p shouldNot] beNil];
//    [[item.dealArray.firstObject.id_p should] equal:@"id"];
//    [[theValue(item.dealArray.firstObject.flr) should] equal:theValue(1.1)];
//});
//
...
//    NSData *data = builder.rawBodyData;
//
//    ORTBOpenrtb *rtb = [[ORTBOpenrtb alloc] initWithData:data error:&error];
//    ADCOMPlacement *adComPlacement = [[ADCOMPlacement alloc] initWithData:rtb.request.itemArray.firstObject.spec.value error:&error];
//
//    [[error should] beNil];
...
//});
//
//it(@"should be valid default field from request model", ^{
//    NSData *data = builder.rawBodyData;
//    NSError *error = nil;
//
//    ORTBOpenrtb *rtb = [[ORTBOpenrtb alloc] initWithData:data error:&error];
...
//    [[theValue(rtb.request.tmax) should] equal:theValue(10000)];
...
//    [targeting stub:@selector(paid) andReturn:theValue(BDMAppPaidVersionPaid)];
//
//    builder.appendRequest(request);
//    NSData *data = builder.rawBodyData;
//
...
//    [[context.device.ifa shouldNot] beNil];
//    [[theValue(context.device.lmt) should] beFalse];
//    [[context.device.mccmnc shouldNot] beNil];
//    [[context.device.carrier shouldNot] beNil];
//    [[theValue(context.device.w) should] equal:theValue(UIScreen.mainScreen.bounds.size.width)];
//    [[theValue(context.device.h) should] equal:theValue(UIScreen.mainScreen.bounds.size.height)];
...
//    [[context.device.hwv shouldNot] beNil];
//
//    if (!request.targeting.restrictions.coppa) {
//        [[theValue(context.device.contype) should] equal:theValue(2)];
//    } else {
//        [[theValue(context.device.contype) should] equal:theValue(0)];
//    }
//});
//
//context(@"user data", ^{
...
//
//    [restriction stub:@selector(coppa) andReturn:theValue(YES)];
//    [restriction stub:@selector(GDPRRequired) andReturn:theValue(YES)];
//
//    builder.appendRequest(request);
//    NSData *data = builder.rawBodyData;
//
